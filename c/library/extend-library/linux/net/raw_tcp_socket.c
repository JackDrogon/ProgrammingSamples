...
int main(int argc, char **argv)
{
	...
	if (setsockopt(sock, IPPROTO_IP, IP_HDRINCL, (char *)&one,
		       sizeof(one)) < 0) {
	...
	ipHdr->check =
	    0; // 16 bit checksum of IP header. Can't calculate at this point
	...
	tcpHdr->dest =
	    htons(dstPort);    // 16 bit in nbp format of destination port
	...
	pTCPPacket.srcAddr = inet_addr(srcIP); // 32 bit format of source
					       // address
	...
	while (1) {
		...
		// Calculate check sum: zero current check, copy TCP header +
		// data to pseudo TCP packet, update check
		tcpHdr->check = 0;

		// Copy tcp header + data to fake TCP header for checksum
		memcpy(pseudo_packet + sizeof(struct pseudoTCPPacket), tcpHdr,
		       sizeof(struct tcphdr) + strlen(data));

		// Set the TCP header's check field
		tcpHdr->check =
		    (csum((unsigned short *)pseudo_packet,
			  (int)(sizeof(struct pseudoTCPPacket) +
				sizeof(struct tcphdr) + strlen(data))));

		printf("TCP Checksum: %d\n", (int)tcpHdr->check);

		// Finally, send packet
		if ((bytes = sendto(sock, packet, ipHdr->tot_len, 0,
