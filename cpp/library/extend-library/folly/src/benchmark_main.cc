...
BENCHMARK(fn_ptr_invoke, iters) { BM_fn_ptr_invoke_impl(iters, doNothing); }
...
BENCHMARK(Function_invoke, iters) { BM_Function_invoke_impl(iters, doNothing); }
...
BENCHMARK(lambda_local_var, iters)
{
	...
	uint32_t count2 = 0;
	BM_invoke_fn_template_impl(iters, [&] {
		// Do something slightly more complicated than just incrementing
		// a variable.  Otherwise gcc is smart enough to optimize the
		// loop away.
		if (count1 & 0x1) {
			++count2;
		}
...
BENCHMARK(std_bind_create_invoke, iters)
{
	...
	for (size_t n = 0; n < iters; ++n) {
		std::function<void()> fn =
		    std::bind(&TestClass::doNothing, &tc);
		fn();
	}
}
...
BENCHMARK(scope_guard_lambda_local_var, iters)
{
	...
	for (size_t n = 0; n < iters; ++n) {
		auto g = makeGuard([&] {
			// Increment count if n is odd.  Without this
			// conditional check (i.e., if we just increment count
			// each time through the loop), gcc is smart enough to
			// optimize the entire loop away, and just set count =
			// iters.
			if (n & 0x1) {
				++count;
			}
	...
	// This check is necessary: if we don't use count, gcc detects that
	// count is unused and optimizes the entire loop away.
