...
public:
	...
	// Assembles the client's payload, sends it and presents the response
	// back from the server.
	std::string SayWorld(const std::string &user)
	{
		// Data we are sending to the server.
		...
		// The actual RPC.
		Status status = stub_->SayWorld(&context, request, &reply);
...
{
public:
	FooClient(std::shared_ptr<Channel> channel)
	    : stub_(Foo::NewStub(channel))
	{
	}
...
int main(int argc, char **argv)
{
	...
	user = "foobar";
	reply = greeter.SayWorld(user);
	std::cout << "Greeter received: " << reply << std::endl;

	FooClient foo(grpc::CreateChannel("localhost:50051",
					  grpc::InsecureChannelCredentials()));
	std::string fuser("fworld");
	std::string freply = foo.SayHello(fuser);
	std::cout << "Greeter received: " << freply << std::endl;

	fuser = "ffoobar";
	freply = foo.SayWorld(fuser);
	std::cout << "Greeter received: " << freply << std::endl;
